...
class InsApp(models.Model):
    ...
    def __str__(self):
        return f"Заявка {self.user.email} на {self.application_type}"  # Изменили строковое представление модели

    def get_passport_info(self):
        return f"Паспорт: {self.passport_data.series_number} {self.passport_data.last_name} {self.passport_data.first_name}"  # Метод для получения информации о паспорте
